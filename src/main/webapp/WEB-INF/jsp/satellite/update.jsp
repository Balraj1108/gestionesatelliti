			...
			<main class="flex-shrink-0">
			  ...
			  <div class='card'>
				    ...
				    <div class='card-body'>
							...
							<form:form modelAttribute="insert_impiegato_attr" method="post" action="${pageContext.request.contextPath}/satellite/updatee" class="row g-3" novalidate="novalidate">
							
							
								<div class="col-md-6">
									<label for="denominazione" class="form-label">denominazione <span class="text-danger">*</span></label>
									<spring:bind path="denominazione">
										<input type="text" name="denominazione" id="denominazione" class="form-control ${status.error ? 'is-invalid' : ''}" placeholder="Inserire il denominazione" value="${insert_impiegato_attr.denominazione }" required>
									</spring:bind>
								...
								<div class="col-md-3">
								    ...
								    <spring:bind path="stato">
									    <select class="form-select ${status.error ? 'is-invalid' : ''}" id="stato" name="stato" required>
									    	...
									    	<option value="IN_MOVIMENTO" ${insert_impiegato_attr.stato == 'IN_MOVIMENTO'?'selected':''}>IN_MOVIMENTO</option>
									      	<option value="FISSO" ${insert_impiegato_attr.stato == 'FISSO'?'selected':''}>FISSO</option>
									      	<option value="DISATTIVATO" ${insert_impiegato_attr.stato == 'DISATTIVATO'?'selected':''}>DISATTIVATO</option>
								    	</select>
								    </spring:bind>
								    <form:errors  path="stato" cssClass="error_field" />
								</div>
								
							<input type="hidden" name="idSatellite" value="${insert_impiegato_attr.id}">
							<div class="col-12">
